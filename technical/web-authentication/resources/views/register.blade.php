<x-layouts.guest :title="__('Register')">
    <x-ui.glass-card padding="p-8" class="reveal" style="animation-delay: 0.08s;">
        <div class="mb-7">
            <h1 class="font-display text-2xl font-bold tracking-tight">{{ __('Créer un compte') }}</h1>
            <p class="mt-1 text-sm text-muted">{{ __('Activez votre command deck personnel.') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('register') }}" class="space-y-5">
            ...
            <div>
                <label for="name" class="mb-1.5 block text-xs font-medium uppercase tracking-wider text-faint">{{ __('Name') }}</label>
            ...
            <div>
                <label for="password_confirmation" class="mb-1.5 block text-xs font-medium uppercase tracking-wider text-faint">{{ __('Confirm Password') }}</label>
                <input id="password_confirmation" type="password" name="password_confirmation" required
                    class="w-full rounded-xl border border-hairline bg-surface/60 px-4 py-3 text-sm text-ink transition placeholder:text-faint focus:border-cyan/50 focus:outline-none focus:ring-2 focus:ring-cyan/20"
                    placeholder="••••••••">
            </div>

            <x-ui.neon-button type="submit" variant="lime" class="w-full">
                {{ __('Créer le compte') }}
        ...
        <p class="mt-6 text-center text-sm text-muted">
            {{ __('Déjà inscrit ?') }}
            <a href="{{ route('login') }}" class="font-medium text-cyan transition hover:text-glow-cyan">{{ __('Se connecter') }}</a>
        </p>
    </x-ui.glass-card>
</x-layouts.guest>
